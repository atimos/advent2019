...
pub fn step2() {
    ...
    let min = get_route(WIRE1)
        .iter()
        .flat_map(|p1| route2.iter().map(move |p2| (p1, p2)))
        .filter(|(p1, p2)| p1 == p2)
...
fn get_route(wire: &str) -> Vec<Point> {
    let mut route = vec![Point::default()];
    for (dir, dist) in get_steps(wire) {
        walk(dir, dist, &mut route);
    }
    route.remove(0);
    route
}

fn get_steps(route: &str) -> Vec<(Dir, i32)> {
    route
        .split(',')
        .map(|path| {
            let (dir, dist) = path.split_at(1);

            match dir {
                "R" => (Dir::Right, dist.parse().unwrap()),
                "L" => (Dir::Left, dist.parse().unwrap()),
                "U" => (Dir::Up, dist.parse().unwrap()),
                "D" => (Dir::Down, dist.parse().unwrap()),
...
fn walk(dir: Dir, dist: i32, path: &mut Vec<Point>) {
    let last = path.last().unwrap().clone();
    match dir {
        Dir::Right => add_x_path(last.x, last.y, std::ops::Add::add, 1..=dist, path),
        Dir::Left => add_x_path(last.x, last.y, std::ops::Sub::sub, 1..=dist, path),
        Dir::Up => add_y_path(last.x, last.y, std::ops::Add::add, 1..=dist, path),
        Dir::Down => add_y_path(last.x, last.y, std::ops::Sub::sub, 1..=dist, path),
    }
}
...
fn add_x_path(
    ...
    steps: std::ops::RangeInclusive<i32>,
    path: &mut Vec<Point>,
) {
    for step in steps {
        path.push(Point { x: op(x, step), y, steps: path.len() as i32 });
    }
}
...
fn add_y_path(
    ...
    for step in steps {
        path.push(Point { x, y: op(y, step), steps: path.len() as i32 });
    }
}
