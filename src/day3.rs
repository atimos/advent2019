...
pub fn part1() {
    let mut wire1 = vec![Point::default()];
    for (dir, dist) in get_steps(WIRE1) {
        walk(dir, dist, &mut wire1);
    }
    wire1.remove(0);

    let mut wire2 = vec![Point::default()];
    for (dir, dist) in get_steps(WIRE2) {
        walk(dir, dist, &mut wire2);
    }
    wire2.remove(0);

    let min = wire1
        .iter()
        .filter(|point| wire2.contains(point))
...
pub fn part2() {
    ...
    let min = wire1
        .iter()
        .map(|p1| (p1, wire2.iter().find(|p2| &p1 == p2)))
        .filter(|(_, p2)| p2.is_some())
        .map(|(p1, p2)| p1.steps + p2.unwrap().steps)
        .min();
